...
if printf '%s' "$CMD" | grep -qE '\b(npm|pnpm|yarn)\b'; then
  ...
  for tok in $pkgs; do
    ...
    if ! [[ "$spec" =~ $EXACT_NPM ]]; then
      hook_deny "npm: '$name@$spec' is not a pinned version (ranges/latest/dist-tags not allowed). Use X.Y.Z."
    fi
    enc="${name/\//%2f}"
...
if printf '%s' "$CMD" | grep -qE '\bpip3?[[:space:]]+install|python3?[[:space:]]+-m[[:space:]]+pip[[:space:]]+install'; then
  ...
  for tok in $reqs; do
    ...
    if [[ "$tok" == *"*"* ]] || [[ "$tok" =~ (\>|\<|~=|!=|,) ]]; then
      hook_deny "pip: '$tok' uses a range/wildcard. Use 'pkg==X.Y.Z' with an exact version."
    fi
    if ! [[ "$tok" =~ ^[A-Za-z0-9._-]+(\[[^]]+\])?==[^*,]+$ ]]; then
      hook_deny "pip: '$tok' is not version-pinned (==X.Y.Z)."
    fi
    pkg="${tok%%[==\[]*}"; ver="${tok##*==}"
...
if printf '%s' "$CMD" | grep -qE '\bgem[[:space:]]+install'; then
  ...
  if [ -z "$gemver" ]; then
    hook_deny "gem: No version specified. Use 'gem install foo -v 1.2.3' with an exact version."
  fi
  if [[ "$gemver" =~ (\>|\<|~|=|,|\*) ]] || ! [[ "$gemver" =~ $EXACT_GEM ]]; then
    hook_deny "gem: '$gemver' is not a pinned version (ranges/operators not allowed)."
  fi
fi
...
if printf '%s' "$CMD" | grep -qE '\bgo[[:space:]]+(get|install)\b'; then
  ...
  for tok in $mods; do
    ...
    [ -z "$tok" ] && continue
    if [[ "$tok" != *"@"* ]]; then
      hook_deny "go: '$tok' has no version/commit hash (equivalent to @latest). Use '@vX.Y.Z' or '@<commit-hash>'."
    fi
    gover="${tok##*@}"
    if [[ "$gover" =~ $GO_HASH ]] || [[ "$gover" =~ $GO_SEMVER ]] || [[ "$gover" =~ $GO_PSEUDO ]]; then
      :
    else
      hook_deny "go: '$tok' is not pinned (@latest/@master/@v-major-only not allowed)."
    fi
  done
  if [ -n "${GOPROXY:-}" ]; then
    if [[ "$GOPROXY" != *golang.flatt.tech* ]]; then
      ...
      add_warn "GOPROXY includes ',direct' which can bypass golang.flatt.tech. Remove direct."
    fi
  fi
fi
