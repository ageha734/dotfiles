...
if printf '%s' "$CMD" | grep -qE 'tg_(anon|org)_[0-9A-Za-z]{8,}'; then
    hook_deny "Guard token (tg_...) found in plaintext in the command. Use a config file or environment variable instead."
fi

if printf '%s' "$CMD" | grep -qE '(curl|wget)\b.*\|\s*(sudo[[:space:]]+)?(bash|sh|zsh)\b'; then
    hook_deny "Piping remote scripts to shell (curl|bash etc.) is blocked. Save the script and review it before execution."
fi
...
if printf '%s' "$CMD" | grep -qE '(cat|head|tail|less|more|xxd|od|strings|grep|awk|sed)\b[^|]*\.(env|pem|key|p12|pfx|tfvars)\b'; then
    hook_warn "Attempting to read a file that may contain secrets (.env/.pem/.key etc.). Do not send its contents to external services."
fi
