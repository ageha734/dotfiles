...
if printf '%s' "$CMD" | grep -qE '\b(npm|pnpm|yarn)\b'; then
    ...
    for tok in $pkgs; do
        ...
        if ! [[ "$spec" =~ $EXACT_NPM ]]; then
            hook_deny "npm: '$name@$spec' is not a pinned version (ranges/latest/dist-tags not allowed). Use X.Y.Z format."
        fi
        enc="${name/\//%2f}"
...
if printf '%s' "$CMD" | grep -qE '\bgem[[:space:]]+install'; then
    ...
    if [ -z "$gemver" ]; then
        hook_deny "gem: No version specified. Use 'gem install foo -v 1.2.3' with a pinned version."
    fi
...
if printf '%s' "$CMD" | grep -qE '\bgo[[:space:]]+(get|install)\b'; then
    ...
    for tok in $mods; do
        ...
        [ -z "$tok" ] && continue
        if [[ "$tok" != *"@"* ]]; then
            hook_deny "go: '$tok' has no version/commit hash (equivalent to @latest). Use '@vX.Y.Z' or '@<commit-hash>' to pin."
        fi
        gover="${tok##*@}"
    ...
    if [ -n "${GOPROXY:-}" ]; then
        if [[ "$GOPROXY" != *golang.flatt.tech* ]]; then
            ...
            add_warn "GOPROXY has ',direct' which can bypass golang.flatt.tech. Remove 'direct' from the proxy list."
        fi
    fi
fi
